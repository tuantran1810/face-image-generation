...
class SequenceDiscriminator(nn.Module):
    def __init__(
        ...
        self.__aenc = AudioEncoder(input_dims = chunk_size, input_channels = 1, output_dims = audio_output_dims)
        self.__fenc = IdentityEncoder(output_channels = frame_output_dims)
        self.__a_gru = nn.GRU(input_size = audio_output_dims, hidden_size = audio_output_dims, num_layers = 1, batch_first = True)
        self.__f_gru = nn.GRU(input_size = frame_output_dims, hidden_size = frame_output_dims, num_layers = 1, batch_first = True)
...
if __name__ == "__main__":
    sd = SequenceDiscriminator(batch_mode = False)
